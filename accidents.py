...
def main():
    try:
        ...
        with open(filename, "rt") as text_file:
            ...
            if perc_reduc < 0:
                raise ValueError(f"{perc_reduc} is too low. Please enter a non-negative number.")
            elif perc_reduc > 100:
                raise ValueError(f"{perc_reduc} is too high. Please enter a valid number between 0 and 100.")
        ...
        if "percent reduction" in str(val_err).lower():
            if "negative" in str(val_err).lower():
                print(f"Error: Percent reduction cannot be negative. Please enter a non-negative number.")
            elif "high" in str(val_err).lower():
                print(f"Error: Percent reduction cannot be more than 100. Please enter a valid number.")
        else:
